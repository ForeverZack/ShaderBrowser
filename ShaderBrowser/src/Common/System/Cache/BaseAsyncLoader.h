...
namespace common
{
	...
	class BaseAsyncLoader
	{
		...
		typedef std::function<DataType*(const std::string&)> CreateFunc;
	...
	public:
		...
		void loadResourceAsync(const std::string& fullpath, CallbackFunc callback)
		{
			...
			// 添加加载任务
			m_pThreadPool->addTask(std::bind(&BaseAsyncLoader<DataType, CallbackFunc>::loadResource, this));
		}
		// 获取加载返回队列
	...
	private:
		...
		void loadResource()
		{
			...
			// 加载数据
			{
				...
				result = m_CreateFunc(asyncData->fullpath.c_str());
	...
	protected:
		...
		std::unordered_map<std::string, AsyncData<DataType, CallbackFunc>*> m_mLoadingAsyncDatas;
	...
	// 加载器 (附带参数)
    // CreateFunc: 资源读取函数类型(通过构造函数将读取方法传入进来); DataType: 数据类型; CallbackFunc: 回调类型; ExtraCreateDataType: 额外的创建参数类型
	template <typename DataType, typename CallbackFunc, typename ExtraCreateDataType = const char*>
	class BaseAsyncLoaderExt
	{
        // Loader默认允许最大线程数
        #define DEFAULT_LOADER_MAX_THREAD_COUNT 2
	...
	public:
		BaseAsyncLoaderExt(CreateFunc createFunc, int maxThreadCount = DEFAULT_LOADER_MAX_THREAD_COUNT)
			: m_CreateFunc(createFunc)
		{
			m_vResponses.clear();
			m_vRequestQueue.clear();
	...
	public:
		...
		void loadResourceAsync(const std::string& fullpath, CallbackFunc callback, shared_ptr<ExtraCreateDataType> extra = nullptr)
		{
            ...
            // 添加加载任务
            m_pThreadPool->addTask(std::bind(&BaseAsyncLoaderExt<DataType, CallbackFunc, ExtraCreateDataType>::loadResource, this));
		}
		// 获取加载返回队列
